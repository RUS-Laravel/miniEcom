                    ...
                    <ol class="dd-list" data-parent="{{ $c->id ?? '' }}">
                        <li class="dd-item" data-cid="{{ $c->id ?? '' }}">
                            ...
                            @foreach ($c->categories as $ca)
                                <ol class="dd-list" data-parent="{{ $ca->id ?? '' }}">
                                    <li class="dd-item" data-cid="{{ $ca->id ?? '' }}">

                                        <div class="dd-handle">
                                            {{ $ca->name ?? '' }} --
                                            <a href="javascript:void()"
                                               data-control="edit-button" data-id="{{ $ca->id }}"
                                               data-url="{{ route('admin.categories.edit', $ca->id) }}">Edit
                                            </a> |
                                            <a href="javascript:void()"
                                               data-control="delete-button" data-id="{{ $ca->id }}"
                                               data-url="{{ route('admin.categories.delete', $ca->id) }}">Delete
                                            </a>
                                        </div>
                                    </li>
                                </ol>
